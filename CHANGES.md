...
* fix an issue where tuples were assumed to be jsonx ast and not checked
...
* add `maps_support/0` call to determine whether `jsonx` was compiled
...
* jsonx is much more pragmatic by default; common json errors are silently
    ignored (and fixed). stricter parsing must be enabled with options
* add support for encoding otp 17.0's new maps data type
* removed `pre_encode` and `post_decode` options in favour of making jsonx
...
* bugfixes involving inappropriate exceptions from jsonx functions
