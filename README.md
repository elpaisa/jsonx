...
**jsonx** is built via [rebar3][rebar3], [rebar][rebar] or [mix][mix] and continuous integration testing provided courtesy [travis-ci][travis]

current status: [![Build Status](https://secure.travis-ci.org/talentdeficit/jsonx.png?branch=develop)](http://travis-ci.org/talentdeficit/jsonx)

**jsonx** is released under the terms of the [MIT][MIT] license
...
{deps, [
       ...
       {jsonx, {git, "https://github.com/talentdeficit/jsonx.git", {branch, "v2.8.0"}}}
]}.
...
```
...
```erlang
1> jsonx:decode(<<"{\"library\": \"jsonx\", \"awesome\": true}">>).
[{<<"library">>,<<"jsonx">>},{<<"awesome">>,true}]
2> jsonx:decode(<<"{\"library\": \"jsonx\", \"awesome\": true}">>, [return_maps]).
#{<<"awesome">> => true,<<"library">> => <<"jsonx">>}
3> jsonx:decode(<<"[\"a\",\"list\",\"of\",\"words\"]">>).
[<<"a">>, <<"list">>, <<"of">>, <<"words">>]
```
...
```erlang
1> jsonx:encode([{<<"library">>,<<"jsonx">>},{<<"awesome">>,true}]).
<<"{\"library\": \"jsonx\", \"awesome\": true}">>
2> jsonx:encode(#{<<"library">> => <<"jsonx">>, <<"awesome">> => true}).
<<"{\"awesome\":true,\"library\":\"jsonx\"}">>
3> jsonx:encode([<<"a">>, <<"list">>, <<"of">>, <<"words">>]).
<<"[\"a\",\"list\",\"of\",\"words\"]">>
```
...
```erlang
1> jsonx:is_json(<<"[\"this is json\"]">>).
true
2> jsonx:is_json("[\"this is not\"]").
false
3> jsonx:is_term([<<"this is a term">>]).
true
4> jsonx:is_term([this, is, not]).
false
```
...
#### to compile **jsonx** so that it always decodes json objects to maps ####
...
**jsonx** is an erlang application for consuming, producing and manipulating 
[json][json]

**jsonx** follows the json [spec][rfc4627] as closely as possible with allowances for
real world usage

**jsonx** is pragmatic. the json spec allows extensions so **jsonx** extends the spec in a
...
objects or arrays. **jsonx** allows a single trailing comma in input. multiple commas
in any posistion or a preceding comma are still errors

all **jsonx** decoder input should be `utf8` encoded binaries. sometimes you get binaries
that are almost but not quite valid utf8 whether due to improper escaping or poor
encoding. **jsonx** replaces invalid codepoints and poorly formed sequences with the 
...
javascript allows them to be delimited by single quotes (`u+0027`) as well. **jsonx**
...
json and **jsonx** only recognize escape sequences as outlined in the json spec. it just
ignores bad escape sequences leaving them in strings unaltered


### migrating from 1.x ###

if you're migrating from jsonx v1.x to v2.x in most cases you won't need to
make any changes to your code

support for otp 17.0's new map type is now enabled by default when compiling
via rebar for any release that supports them. jsonx should still compile cleanly for
earlier releases without any user intervention

if you used any of `replaced_bad_utf8`, `single_quoted_strings`, `comments`,
`ignored_bad_escapes` or `relax` you can simply omit them from your calls to jsonx,
they are all enabled by default now. if you want stricter parsing see the new
[`strict` options](#option) available

if you were using jsonx to parse partial json using it's streaming features it is now
disabled by default. you'll need to pass the `stream` option to calls to jsonx functions
to reenable it
...
*   numbers
    ...
    whenever possible, **jsonx** will interpret json numbers that look like integers as 
...
*   strings
    ...
    because **jsonx** only accepts `utf8` binaries all binary strings must be `utf8`.
...
**jsonx** can handle incomplete json texts. if the option `stream` is passed to the decoder
or parser and if a partial json text is parsed, rather than returning a term from
your callback handler, **jsonx** returns `{incomplete, F}` where  `F` is a function with 
...
however, it is important to recognize that **jsonx** is conservative by default. **jsonx** will 
...
**jsonx** functions all take a common set of options. not all flags have meaning 
...
- `strict`
    ...
    as mentioned [earlier](#description), **jsonx** is pragmatic. if you're more of a
    ...
    * `escapes`
    ...
    any combination of these can be passed to **jsonx** by using `{strict, [strict_option()]}`.
...
**jsonx** is a json compiler with interleaved tokenizing, syntactic analysis and 
...
if **jsonx** was compiled with map support enabled returns `true`, else
`false`


## callback exports ##

the following functions should be exported from a **jsonx** callback module
